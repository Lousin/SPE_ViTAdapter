...
import sys
import os
sys.path.append(os.path.abspath('/root/shangqi/code/spe_rwkv'))
...
def main():
    ...
    if args.load_masks:
        maskdir = os.path.join(datadir, domain,
            f'masks_{int(cfg.DATASET.INPUT_SIZE[0]/cfg.DATASET.MASK_RATIO)}x{int(cfg.DATASET.INPUT_SIZE[1]/cfg.DATASET.MASK_RATIO)}')
    ...
    for idx in tqdm(range(len(labels))):
        ...
        if args.load_masks:
            ...
            mask_out = cv2.resize(mask, [int(s / cfg.DATASET.MASK_RATIO) for s in cfg.DATASET.INPUT_SIZE])
